        ...
        <div className="glass-panel rounded-[32px] border border-white/10 p-6 md:p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-bold text-white">Yêu cầu liên hệ từ danh bạ công khai</h1>
                ...
                >
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div className="space-y-3">
                      ...
                      {item.message && <p className="max-w-3xl text-sm leading-7 text-white/75">{item.message}</p>}
                      ...
                      {item.therapist_reply && (
                        <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/70">
                          <div className="mb-1 text-xs uppercase tracking-[0.25em] text-white/35">Phản hồi của bạn</div>
                          {item.therapist_reply}
                        </div>
                      )}

                      {item.shared_pairing_code && (
                        <div className="inline-flex items-center gap-2 rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-100">
                          <LinkIcon size={16} />
                      ...
                      {item.status === 'pending' ? (
                        <>
                          <button
                            ...
                            className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-white/10 disabled:opacity-60"
                          >
                            <Check size={16} />
                            Chấp nhận
                          </button>
                          ...
                          <button
                            ...
                            disabled={busyId === item.id}
                            className="inline-flex items-center gap-2 rounded-2xl border border-red-400/20 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-100 disabled:opacity-60"
                          >
                            <X size={16} />
                            Từ chối
                          </button>
                        ...
                        <button
                          ...
                          disabled={busyId === item.id}
                          className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white/75 transition-colors hover:bg-white/10 disabled:opacity-60"
                        >
                          Lưu trữ
                        </button>
                      )}
