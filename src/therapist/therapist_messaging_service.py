...
class TherapistMessagingService:
    ...
    def get_recent_conversations(self, therapist_id: str, limit: int = 20) -> List[Dict]:
        ...
        # Lấy tin nhắn mới nhất của mỗi client (không join vì client_id là TEXT)
        response = self.supabase.table('therapist_client_messages')\
            .select('''
                client_id,
                message_content,
        ...
        if client_ids:
            ...
            for client_id, conv in conversations.items():
                conv['client'] = users_map.get(client_id, {
                    'id': client_id,
                    'name': 'Unknown',
                    'email': '',
                    'picture': None
