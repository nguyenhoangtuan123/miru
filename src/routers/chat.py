    ...
    try:
        while True:
            ...
            try:
                ...
                for index, chunk in enumerate(_stream_chunks(ai_message)):
                    await websocket.send_json({
                        "type": "ai_chunk",
                        "chunk": chunk,
                        "index": index,
                ...
                try:
                    if memory_service:
                        ...
                        if session_id:
                            memory_service.update_session_facts_background(session_id, user_id, user_message, ai_message)
                except Exception as e:
                ...
                try:
                    get_push_service().send_push_to_user(
                        user_id=user_id,
                        title="Miru vua phan hoi",
                        body=ai_message[:160],
                        url="/chat",
                        tag="chat-ai-response",
                        extra={"kind": "chat_response", "session_id": session_id},
                    )
                except Exception as e:
                    print(f"[Push] Failed to send AI response push: {e}")
