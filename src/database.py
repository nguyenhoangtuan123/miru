...
class DatabaseManager:
    ...
    def _is_session_metadata_summary(self, summary_text: str) -> bool:
        if not summary_text:
            return False
        ...
        if data.get("_record_type") == "chat_session":
            return True

        metadata_keys = {"title", "created_at", "status", "message_count"}
        return metadata_keys.issubset(set(data.keys()))

    def _filter_memory_records(self, rows):
        return [row for row in (rows or []) if not self._is_session_metadata_summary(row.get("summary_text"))]
    ...
    def find_relevant_summaries(self, user_id: str, query_text: str, n_results: int = 3):
        ...
        match_count = max(n_results * 5, n_results)
        ...
        if response.data:
            results = []
            filtered_rows = self._filter_memory_records(response.data)
    ...
    def get_latest_session_summary(self, user_id: str):
        ...
        if response.data:
            ...
            if not filtered_rows:
                return "ChÆ°a cÃ³ kÃ½ á»©c nÃ o."

            item = filtered_rows[0]
            text = item['summary_text']
            created_str = item.get('created_at')
    ...
    def analyze_patterns_simple(self, user_id: str, keyword: str):
        ...
        total_response = self.supabase.table('session_summaries').select('summary_text').eq('user_id', user_db_id).execute()
        filtered_rows = self._filter_memory_records(total_response.data)
        total_sessions = len(filtered_rows)
        if total_sessions == 0:
            return "Chưa có dữ liệu để phân tích."

        count = sum(1 for row in filtered_rows if keyword.lower() in row.get('summary_text', '').lower())
