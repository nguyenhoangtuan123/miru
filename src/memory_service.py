...
def _build_mem0_file_handler(log_path: str):
    try:
        handler = logging.FileHandler(log_path, encoding="utf-8")
        handler.setLevel(logging.DEBUG)
        return handler
    except PermissionError as exc:
        print(f"[mem0] Could not open log file '{log_path}': {exc}. Falling back to console logging only.")
        return None
...
# File handler - ghi log vào file ở project root để giữ nguyên vị trí runtime data
log_file = os.path.join(PROJECT_ROOT, "mem0.log")
file_handler = _build_mem0_file_handler(log_file)
...
class MemoryService:
    ...
    def _get_session_lock(self, session_id: Any) -> threading.RLock:
        normalized_session_id = self._normalize_session_id(session_id)
        with _session_facts_locks_guard:
            lock = _session_facts_locks.get(normalized_session_id)
            if lock is None:
    ...
    def _memory_belongs_to_user(self, user_id: str, memory_id: str) -> bool:
        ...
        memories = self.get_all_memories(user_id).get("results", [])
        return any(str(memory.get("id")) == str(memory_id) for memory in memories)
    ...
    def delete_memory(self, memory_id: str, user_id: Optional[str] = None) -> bool:
        ...
        try:
            if self.memory is None:
                return False
            if user_id and not self._memory_belongs_to_user(user_id, memory_id):
                print(f"[DELETE] Mem0: Memory {memory_id} does not belong to user {user_id}")
                return False
    ...
    def update_memory(self, memory_id: str, new_data: str, user_id: Optional[str] = None) -> bool:
        """
        ...
        try:
            if self.memory is None:
                return False
            if user_id and not self._memory_belongs_to_user(user_id, memory_id):
                print(f"[UPDATE] Mem0: Memory {memory_id} does not belong to user {user_id}")
                return False
    ...
    def ensure_session_dir(self, session_id: Any) -> str:
        ...
        base_dir = os.path.join(PROJECT_ROOT, "memories")
        normalized_session_id = self._normalize_session_id(session_id)
        session_dir = os.path.join(base_dir, f"session_{normalized_session_id}")
...
"""
    ...
    def update_session_facts_background(self, session_id: Any, user_id: str, user_input: str, ai_response: str):
        """Cập nhật facts.txt trong luồng phụ (không block user)"""
        try:
            self._normalize_session_id(session_id)
        except ValueError:
            mem0_logger.warning("Skipping facts background update because session_id is missing")
            return

        thread = threading.Thread(
            target=self._update_facts_sync, 
            args=(session_id, user_id, user_input, ai_response)
        )
        thread.daemon = True
        thread.start()

    def _update_facts_sync(self, session_id: Any, user_id: str, user_input: str, ai_response: str):
        ...
        if needs_consolidation and user_id:
            self.consolidate_memory_cycle(session_id, user_id)
            needs_consolidation = False
        elif needs_consolidation:
            needs_consolidation = False
        
        if False and needs_consolidation:
...
"""
    ...
    def consolidate_memory_cycle(self, session_id: Any, user_id: str):
        ...
        if not user_id:
            print(f"[SKIP] Cannot consolidate session {session_id} without user_id")
            return False
...
"""
    ...
    def sync_session_facts_to_mem0(self, session_id: Any, user_id: str) -> bool:
        ...
        try:
            ...
            print("[SYNC] Skipping sync because session_id is missing")
            return False
            
        print(f"[SYNC] Starting sync for session {normalized_session_id} to Mem0...")
        try:
            facts_content = self.get_session_facts(normalized_session_id)
            ...
            memory_text = f"""
Summary of Session {normalized_session_id} with User {user_id}:
...
"""
            ...
            existing_id = None
            all_memories = self.get_all_memories(user_id).get("results", [])
            for mem in all_memories:
                metadata = mem.get('metadata') or {}
                if str(metadata.get('session_id')) == normalized_session_id and metadata.get('type') == 'session_summary':
                    existing_id = mem['id']
                    break
            ...
            if existing_id:
                ...
                self.memory.add(
                    ...
                    user_id=user_id,
                    metadata={"session_id": normalized_session_id, "type": "session_summary", "source": "facts_sync"}
                )
